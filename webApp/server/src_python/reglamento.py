...
class Reglamento:
	...
	def accesibleParaCaballo(self, __d, player, matrizBoard):
		...
		#print("Revisando en: " + str((int(__d[0]) + 2)) + str((int(__d[1]) + 1)) + "\nTipo: " + matrizBoard[(int(__d[0]) + 2)][(int(__d[1]) + 1)].piece_type)
		#print("Revisando en: " + str((int(__d[0]) + 2)) + str((int(__d[1]) - 1)) + "\nTipo: " + matrizBoard[(int(__d[0]) + 2)][(int(__d[1]) - 1)].piece_type)
		#print("Revisando en: " + str((int(__d[0]) - 2)) + str((int(__d[1]) + 1)) + "\nTipo: " + matrizBoard[(int(__d[0]) - 2)][(int(__d[1]) + 1)].piece_type)
		#print("Revisando en: " + str((int(__d[0]) - 2)) + str((int(__d[1]) - 1)) + "\nTipo: " + matrizBoard[(int(__d[0]) - 2)][(int(__d[1]) - 1)].piece_type)
		#print("Revisando en: " + str((int(__d[0]) + 1)) + str((int(__d[1]) + 2)) + "\nTipo: " + matrizBoard[(int(__d[0]) + 1)][(int(__d[1]) + 2)].piece_type)
		#print("Revisando en: " + str((int(__d[0]) + 1)) + str((int(__d[1]) - 2)) + "\nTipo: " + matrizBoard[(int(__d[0]) + 1)][(int(__d[1]) - 2)].piece_type)
		#print("Revisando en: " + str((int(__d[0]) - 1)) + str((int(__d[1]) + 2)) + "\nTipo: " + matrizBoard[(int(__d[0]) - 1)][(int(__d[1]) + 2)].piece_type)
		#print("Revisando en: " + str((int(__d[0]) - 1)) + str((int(__d[1]) - 2)) + "\nTipo: " + matrizBoard[(int(__d[0]) - 1)][(int(__d[1]) - 2)].piece_type)

		#matrizBoard[(int(__d[0]) + 2)][(int(__d[1]) - 1)] = Queen()
		#matrizBoard[(int(__d[0]) - 2)][(int(__d[1]) - 1)] = Queen()
		#matrizBoard[(int(__d[0]) + 1)][(int(__d[1]) - 2)] = Queen()
		#matrizBoard[(int(__d[0]) - 1)][(int(__d[1]) - 2)] = Queen()
	...
	def moverPeon(self, movement, player, matrizBoard):
		...
		if response: # Este bloque realiza la promoción del Peon.
			if self.is_capture(movement, 'P'):
				print("Line 647.")
				if len(movement) == 5: # La longitud requerida para que sea una promoción.
					if movement[4] in matrizInicialesPiezas:
						matrizBoard[int(response[0])][int(response[1])].piece_type = movement[4]
				...
				if len(movement) == 4 and movement[3] in matrizInicialesPiezas:
			...
			if self.is_capture(movement, 'P'):
				if len(movement) == 8: # La longitud requerida para que sea una captura al paso.
					if movement[4] == 'a' and movement[5] == '.' and movement[6] == 'p' and movement[7] == '.':
						print("Line 665.\nes captura al paso...")
						c = self.__matrizColumns.index(movement[0])
						print("La columna de origen es: " + str(c))
						if player == "w":
							if (int(__d[0]) + 1) < 8:
								print("Revisando en: " + str(int(__d[0]) + 1) + str(c))
								if matrizBoard[int(__d[0]) + 1][c].piece_type == "P" and matrizBoard[int(__d[0]) + 1][c].color == "w":
									if matrizBoard[int(__d[0]) + 1][int(__d[1])].piece_type == "P" and matrizBoard[int(__d[0]) + 1][int(__d[1])].color == "b":
										if matrizBoard[int(__d[0]) + 1][int(__d[1])].ultimateMovementWas(self,"double_movement"):
											matrizBoard[int(__d[0]) + 1][int(__d[1])] = Square()
											response = str(int(__d[0]) + 1) + str(c) + " " + __d
						else:
							if (int(__d[0]) - 1) > -1:
								print("Revisando en: " + str(int(__d[0]) - 1) + str(c))
								if matrizBoard[int(__d[0]) - 1][c].piece_type == "P" and matrizBoard[int(__d[0]) - 1][c].color == "b":
									if matrizBoard[int(__d[0]) - 1][int(__d[1])].piece_type == "P" and matrizBoard[int(__d[0]) - 1][int(__d[1])].color == "w":
										# En cuanto esté solucionado el problema del historial, debería verify.
										if matrizBoard[int(__d[0]) - 1][int(__d[1])].ultimateMovementWas(self, "double_movement"):
											matrizBoard[int(__d[0]) - 1][int(__d[1])] = Square()
											response = str(int(__d[0]) - 1) + str(c) + " " + __d
